...
func (h *MediaServerHandler) ListMediaServers(c *gin.Context) {
	...
	query := h.db.Model(&database.MediaServer{})
...
func (h *MediaServerHandler) DeleteMediaServer(c *gin.Context) {
	...
	// 执行删除
	result := h.db.Delete(&database.MediaServer{}, uint(id))
	if result.Error != nil {
...
func testEmbyConnection(server database.MediaServer, logger *zap.Logger) ConnectionTestResult {
	start := time.Now()
...
func testJellyfinConnection(server database.MediaServer, logger *zap.Logger) ConnectionTestResult {
	start := time.Now()
	apiURL := fmt.Sprintf("http://%s:%d/System/Info/Public", server.Host, server.Port)
...
func testPlexConnection(server database.MediaServer, logger *zap.Logger) ConnectionTestResult {
	start := time.Now()
