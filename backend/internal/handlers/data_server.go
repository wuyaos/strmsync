...
import (
	...
	clouddrive2 "github.com/strmsync/strmsync/internal/clients/clouddrive2"
...
func (h *DataServerHandler) ListDataServers(c *gin.Context) {
	...
	query := h.db.Model(&database.DataServer{})
	...
	// 查询列表
	var servers []database.DataServer
...
func (h *DataServerHandler) GetDataServer(c *gin.Context) {
	...
	var server database.DataServer
...
func (h *DataServerHandler) DeleteDataServer(c *gin.Context) {
	...
	// 执行删除
	result := h.db.Delete(&database.DataServer{}, uint(id))
	if result.Error != nil {
...
// 来验证服务器连接和认证信息
func testCloudDrive2Connection(server database.DataServer, logger *zap.Logger) ConnectionTestResult {
	start := time.Now()
...
func testOpenListConnection(server database.DataServer, logger *zap.Logger) ConnectionTestResult {
	start := time.Now()
