...
// Planner 同步计划器实现
type Planner struct {
	dataServerClient service.DataServerClient
}

// NewPlanner 创建同步计划器
func NewPlanner(dataServerClient service.DataServerClient) service.SyncPlanner {
	return &Planner{
...
// Plan 根据文件事件生成同步计划
func (p *Planner) Plan(ctx context.Context, config *types.JobConfig, events <-chan types.FileEvent) (<-chan types.SyncPlanItem, <-chan error) {
	itemCh := make(chan types.SyncPlanItem)
...
func (p *Planner) planItem(ctx context.Context, config *types.JobConfig, event *types.FileEvent) (*types.SyncPlanItem, error) {
	...
	switch event.Type {
	case types.FileEventCreate:
		op = types.SyncOpCreate
	case types.FileEventUpdate:
		op = types.SyncOpUpdate
	case types.FileEventDelete:
		op = types.SyncOpDelete
	default:
