...
// Client 媒体服务器客户端实现
type Client struct {
	serverType types.MediaServerType
	baseURL    *url.URL
	apiKey     string
...
func NewClient(config types.MediaServerConfig, opts ...Option) (service.MediaServerClient, error) {
	// 验证服务器类型
...
func (c *Client) Scan(ctx context.Context, libraryPath string) error {
...
func (c *Client) endpointPath(path string) string {
	if !strings.HasPrefix(path, "/") {
		path = "/" + path
	}

	switch c.serverType {
	case types.MediaServerTypeEmby:
		return "/emby" + path
	case types.MediaServerTypeJellyfin:
		return path
	default:
		return path
	}
...
// setAuthHeader 设置认证头（Emby和Jellyfin使用不同的认证方式）
func (c *Client) setAuthHeader(req *http.Request) {
	switch c.serverType {
	case types.MediaServerTypeEmby:
		// Emby 使用 X-Emby-Token header
		req.Header.Set("X-Emby-Token", c.apiKey)
	case types.MediaServerTypeJellyfin:
		// Jellyfin 使用 MediaBrowser 认证格式
...
// do 执行HTTP请求并验证响应状态码
func (c *Client) do(req *http.Request, expectedStatuses ...int) error {
