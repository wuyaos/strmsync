...
// Apply 执行同步计划
func (g *Generator) Apply(ctx context.Context, items <-chan types.SyncPlanItem) (succeeded int, failed int, err error) {
	for {
		select {
		case <-ctx.Done():
...
func (g *Generator) createOrUpdateStrm(ctx context.Context, item *types.SyncPlanItem) error {
	// 1. 确保目标目录存在
...
// deleteStrm 删除strm文件
func (g *Generator) deleteStrm(ctx context.Context, item *types.SyncPlanItem) error {
	// 1. 检查文件是否存在
