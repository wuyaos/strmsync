...
// NewPlanner 创建同步计划器
func NewPlanner(dataServerClient filesystem.Client) SyncPlanner {
	return &Planner{
...
// Plan 根据文件事件生成同步计划
func (p *Planner) Plan(ctx context.Context, config *JobConfig, events <-chan FileEvent) (<-chan SyncPlanItem, <-chan error) {
	itemCh := make(chan SyncPlanItem)
...
func (p *Planner) planItem(ctx context.Context, config *JobConfig, event *FileEvent) (*SyncPlanItem, error) {
