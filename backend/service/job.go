...
type jobService struct {
	...
	runningJobs     map[JobID]*ExecutionContext
	runningJobsLock sync.RWMutex
}

// NewService 创建Job服务
func NewJobService(db *gorm.DB, taskRunService TaskRunService, taskExecutor TaskExecutor) JobService {
	return &jobService{
		db:             db,
		taskRunService: taskRunService,
		taskExecutor:   taskExecutor,
		runningJobs:    make(map[JobID]*ExecutionContext),
	}
}

// Run 运行任务
func (s *jobService) Run(ctx context.Context, jobID JobID) (TaskRunID, error) {
...
// Stop 停止任务
func (s *jobService) Stop(ctx context.Context, jobID JobID) error {
...
// Validate 验证Job配置
func (s *jobService) Validate(ctx context.Context, jobID JobID) error {
...
func (s *jobService) GetRunningTaskRun(ctx context.Context, jobID JobID) (TaskRunID, bool, error) {
	s.runningJobsLock.RLock()
...
func (s *jobService) loadAndValidateJob(ctx context.Context, jobID JobID) (*JobConfig, error) {
	...
	config := &JobConfig{
		ID:            job.ID,
		Name:          job.Name,
		WatchMode:     WatchMode(job.WatchMode),
