...
type TaskQueue interface {
	...
	//   - error: 领取失败时返回错误
	ClaimNext(ctx context.Context, workerID string) (*core.TaskRun, error)
...
type DataServerRepository interface {
	...
	//
	// 返回：
	//   - core.DataServer: DataServer 对象
	//   - error: 查询失败或不存在时返回错误
	GetByID(ctx context.Context, id uint) (core.DataServer, error)
}
...
type DriverFactory interface {
	...
	//   - error: 构建失败时返回错误
	Build(ctx context.Context, server core.DataServer) (syncengine.Driver, error)
}
...
type WriterFactory interface {
	...
	//   - error: 构建失败时返回错误
	Build(ctx context.Context, job core.Job) (syncengine.Writer, error)
}
