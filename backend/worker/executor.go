...
func NewExecutor(cfg ExecutorConfig) (*Executor, error) {
	...
	// 设置日志器
	if cfg.Logger == nil {
		cfg.Logger = utils.With(zap.String("component", "worker-executor"))
	}
...
// - 配置错误返回永久失败
// - 执行错误根据类型返回可重试或永久失败
func (e *Executor) Run(ctx context.Context, task *core.TaskRun) (syncengine.SyncStats, error) {
	if e == nil {
...
func buildEngineOptions(job core.Job) (syncengine.EngineOptions, error) {
...
func (f DefaultDriverFactory) Build(ctx context.Context, server core.DataServer) (syncengine.Driver, error) {
	if ctx == nil {
		ctx = context.Background()
	}
...
func (f DefaultDriverFactory) logger() *zap.Logger {
	...
	return utils.With(zap.String("component", "worker-driver-factory"))
}
...
func (f DefaultWriterFactory) Build(ctx context.Context, job core.Job) (syncengine.Writer, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	if strings.TrimSpace(job.TargetPath) == "" {
...
// - Username: 用户名
// - Password: 密码
func buildFilesystemConfig(server core.DataServer) (filesystem.Config, error) {
	// 解析 Options JSON
...
func (r *GormTaskRunRepository) UpdateProgress(ctx context.Context, taskID uint, progress TaskRunProgress) error {
	...
	return r.db.WithContext(ctx).Model(&core.TaskRun{}).
		Where("id = ?", taskID).
