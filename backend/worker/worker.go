...
func NewWorker(cfg WorkerConfig) (*WorkerPool, error) {
	...
	// 设置默认值
	if cfg.Logger == nil {
		cfg.Logger = utils.With(zap.String("component", "worker"))
	}
	...
	if cfg.WorkerID == "" {
		cfg.WorkerID = "worker-" + utils.NewRequestID()
	}
...
func (w *WorkerPool) claimNext() (*core.TaskRun, error) {
	claimCtx := w.ctx
...
func (w *WorkerPool) executeTask(log *zap.Logger, task *core.TaskRun) error {
	if task == nil {
		return nil
	}
