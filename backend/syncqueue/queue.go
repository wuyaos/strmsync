...
func NewSyncQueue(db *gorm.DB) (*SyncQueue, error) {
	...
	return &SyncQueue{
		db:  db,
		log: utils.With(zap.String("component", "syncqueue")),
	}, nil
}
...
//
// 返回：
//   - *core.TaskRun: 领取到的任务，如果没有可用任务则返回 nil
//   - error: 领取失败时返回错误
func (q *SyncQueue) ClaimNext(ctx context.Context, workerID string) (*core.TaskRun, error) {
	if q == nil || q.db == nil {
...
// 返回：
//   - error: 操作失败时返回错误
func (q *SyncQueue) Enqueue(ctx context.Context, task *core.TaskRun) error {
	if q == nil || q.db == nil {
		return fmt.Errorf("syncqueue: db not initialized")
	}
	if task == nil {
...
//   - []core.TaskRun: 任务列表
//   - error: 查询失败时返回错误
func (q *SyncQueue) List(ctx context.Context, filter TaskFilter) ([]core.TaskRun, error) {
	if q == nil || q.db == nil {
		return nil, fmt.Errorf("syncqueue: db not initialized")
	}
	if ctx == nil {
	...
	db := q.db.WithContext(ctx).Model(&core.TaskRun{})
