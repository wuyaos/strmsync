// Package core 提供 Job 相关的 GORM Repository 实现
package core
...
// GormJobRepository 是基于 GORM 的 Job 数据访问实现
//
// 此实现提供了 scheduler 和 worker 包所需的所有 Job 查询和更新方法。
...
//
// 仅返回 Enabled=true 的 Job，按 ID 升序排序。
//
// 参数：
//   - ctx: 上下文（为 nil 时自动使用 Background）
//
...
// UpdateLastRunAt 更新 Job 的 last_run_at 字段
//
// 用于记录任务的最后成功执行时间。
//
// 参数：
