...
func NewScheduler(cfg SchedulerConfig) (*CronScheduler, error) {
	...
	// 设置默认值
	if cfg.Logger == nil {
		cfg.Logger = utils.With(zap.String("component", "scheduler"))
	}
	if cfg.Location == nil {
...
//
// 并发安全：通过 mu 保护所有映射操作
func (s *CronScheduler) UpsertJob(ctx context.Context, job core.Job) error {
	if s == nil {
		return fmt.Errorf("scheduler: nil receiver")
	}
	if !s.running.Load() {
...
func (s *CronScheduler) registerJobLocked(job core.Job) error {
	if s.cron == nil {
		return fmt.Errorf("scheduler: cron engine not initialized")
	}
...
// - schedule_time: 调度时间
// - cron_spec: Cron 表达式
func (s *CronScheduler) buildPayload(job core.Job, scheduleTime time.Time) string {
