# STRMSync 开发脚本使用指南

## 📂 脚本列表

| 脚本 | 说明 | 用途 |
|------|------|------|
| `dev-start.sh` | 启动开发环境 | 前后端一起启动（测试模式） |
| `dev-stop.sh` | 停止开发环境 | 停止所有进程并清理端口 |
| `dev-restart.sh` | 重启开发环境 | 先停止再启动（解决端口占用） |

---

## 🚀 快速开始

### 1. 启动开发环境

```bash
cd /path/to/strm
./scripts/dev-start.sh
```

**特性**：
- ✅ 自动加载 `tests/.env.test` 配置
- ✅ 在 `tests/` 目录运行（隔离测试环境）
- ✅ 后端端口：`3000`（由 .env.test 配置）
- ✅ 前端端口：`5676`
- ✅ 自动检查端口占用
- ✅ 等待服务启动并验证健康状态
- ✅ 后台运行，日志输出到文件

**启动后访问**：
- 🎨 前端界面：http://localhost:5676
- 🔧 后端 API：http://localhost:3000/api/health

**日志查看**：
```bash
# 查看后端日志
tail -f tests/logs/backend.log

# 查看前端日志
tail -f tests/logs/frontend.log
```

---

### 2. 停止开发环境

```bash
./scripts/dev-stop.sh
```

**功能**：
- ✅ 根据 PID 文件停止后端和前端
- ✅ 等待进程优雅退出（10秒超时）
- ✅ 超时后强制 kill -9
- ✅ 清理所有端口占用（包括遗留进程）
- ✅ 删除 PID 文件

# 启动生产服务（工作目录为 dist）
./dist/prod-start.sh

### 3. 重启开发环境

```bash
./scripts/dev-restart.sh
```

**适用场景**：
- 🔄 修改代码后需要重启
- 🐛 进程僵死需要强制重启
- ⚠️ 端口被占用无法启动

**等效于**：
```bash
./scripts/dev-stop.sh
sleep 1
./scripts/dev-start.sh
```

**环境变量配置：**
- 在项目根目录创建 `.env` 文件
- 参考 `.env.example` 配置生产环境参数
- `make build` 会将 `.env` 复制到 `dist/.env`（若不存在则复制 `.env.example`）

测试环境配置，主要参数：

```bash
# 后端端口（与生产环境隔离）
PORT=3000

# 测试数据库（独立于生产数据）
DB_PATH=data/test.db

# 调试日志
LOG_LEVEL=debug

# 测试用加密密钥
ENCRYPTION_KEY=test_encryption_key_12345678
```

### frontend/vite.config.js

前端会根据环境变量动态配置后端代理：

```javascript
proxy: {
  '/api': {
    target: process.env.VITE_BACKEND_PORT
      ? `http://localhost:${process.env.VITE_BACKEND_PORT}`
      : 'http://localhost:6754',  // 默认生产端口
    changeOrigin: true
  }
}
```

---

## 📁 目录结构

```
strm/
├── scripts/
│   ├── dev-start.sh      # 启动脚本
│   ├── dev-stop.sh       # 停止脚本
│   └── dev-restart.sh    # 重启脚本
│
├── tests/
│   ├── .env.test         # 测试环境配置
│   ├── data/             # 测试数据库（自动创建）
│   ├── logs/             # 日志目录（自动创建）
│   │   ├── backend.log   # 后端日志
│   │   └── frontend.log  # 前端日志
│   ├── backend.pid       # 后端进程 PID
│   └── frontend.pid      # 前端进程 PID
│
├── backend/
│   └── .env              # 临时：复制自 tests/.env.test
│
└── frontend/
    └── vite.config.js    # 支持环境变量配置
```

---

## 🔧 故障排查

### 问题 1：端口被占用

**现象**：
```
⚠ 警告: 后端 端口 3000 已被占用
✗ 端口检查失败，请先停止占用端口的进程
```

**解决**：
```bash
# 方案1：使用重启脚本（推荐）
./scripts/dev-restart.sh

# 方案2：手动清理
./scripts/dev-stop.sh
./scripts/dev-start.sh

# 方案3：手动查找并杀死进程
lsof -ti :3000 | xargs kill -9
lsof -ti :5676 | xargs kill -9
```

---

### 问题 2：后端启动超时

**现象**：
```
等待后端就绪......✗
✗ 后端启动超时，查看日志: tests/logs/backend.log
```

**排查**：
```bash
# 查看后端日志
cat tests/logs/backend.log

# 常见原因：
# - 数据库文件权限问题
# - .env.test 配置错误
# - Go 依赖未安装

# 手动启动后端调试
cd backend
go run ./cmd/server
```

---

### 问题 3：前端启动超时

**现象**：
```
等待前端就绪......✗
✗ 前端启动超时，查看日志: tests/logs/frontend.log
```

**排查**：
```bash
# 查看前端日志
cat tests/logs/frontend.log

# 常见原因：
# - npm 依赖未安装
# - node 版本过低（需要 18+）
# - vite 端口被占用

# 手动启动前端调试
cd frontend
npm install
npm run dev
```

---

### 问题 4：进程无法停止

**现象**：停止脚本执行后进程仍在运行

**解决**：
```bash
# 强制停止所有相关进程
pkill -9 -f "strmsync"
pkill -9 -f "cmd/server"
pkill -9 -f "vite.*frontend"

# 清理端口占用
lsof -ti :3000 | xargs kill -9
lsof -ti :5676 | xargs kill -9
```

---

## 📝 注意事项

1. **测试环境隔离**
   - 测试环境使用独立端口（3000）和数据库（data/test.db）
   - 不会影响生产环境配置（端口 6754，app/data.db）

2. **日志持久化**
   - 后台运行的日志保存在 `tests/logs/` 目录
   - 每次启动不会清空旧日志（需手动清理）

3. **PID 文件管理**
   - PID 文件用于进程管理，停止脚本会自动清理
   - 如果异常退出导致 PID 文件残留，可手动删除：
     ```bash
     rm -f tests/*.pid
     ```

4. **环境变量传递**
   - `dev-start.sh` 会设置 `VITE_BACKEND_PORT` 环境变量
   - Vite 会读取该变量并配置代理到测试环境后端

5. **数据持久性**
   - 测试数据库 `tests/data/test.db` 会持久保存
   - 如需全新测试环境，删除数据库即可：
     ```bash
     rm -f tests/data/test.db
     ```

---

## 🎯 典型工作流

### 日常开发

```bash
# 1. 启动开发环境
./scripts/dev-start.sh

# 2. 访问前端进行测试
open http://localhost:5676

# 3. 查看实时日志（可选）
tail -f tests/logs/backend.log

# 4. 修改代码后重启
./scripts/dev-restart.sh

# 5. 结束开发，停止服务
./scripts/dev-stop.sh
```

### 调试后端

```bash
# 1. 停止开发环境
./scripts/dev-stop.sh

# 2. 手动启动前端
cd frontend
npm run dev &

# 3. 手动启动后端（可打断点）
cd ../backend
cp ../tests/.env.test .env
go run ./cmd/server

# 4. 调试完成后全部停止
./scripts/dev-stop.sh
```

---

## 📚 相关文档

- [HTTP API 文档](../docs/HTTP_API.md)
- [部署文档](../docs/DEPLOYMENT.md)
- [测试指南](../TESTING_GUIDE.md)

---

**最后更新**: 2026-02-19
**维护者**: STRMSync Team
